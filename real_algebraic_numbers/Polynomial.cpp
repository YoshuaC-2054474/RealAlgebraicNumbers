...
Polynomial::Polynomial(int zeroCoeff) {
	std::vector<Rational> coeffs = {zeroCoeff};
	coefficients = coeffs;
...
static std::vector<int> getPossibleRoots(const std::vector<Rational>& poly) {
	...
	const int constant = poly[0].toInt();
	const int leading = poly.back().toInt();
	...
	if (lowestNz > 0) {
		...
		// Check if both exponents are divisible by 3 (check sum/difference of cubes)
			...
			if (nonZeroTerms[0].first.isInteger() && nonZeroTerms[1].first.isInteger()) {
				int coeff1 = nonZeroTerms[0].first.toInt();
				int coeff2 = nonZeroTerms[1].first.toInt();
